...
} = require("@mui/internal-api-docs-builder");
...
const {
  default: CORE_TYPESCRIPT_PROJECTS,
} = require("../../scripts/coreTypeScriptProjects");
...
async function transpileFile(tsxPath, project) {
  ...
  try {
    ...
    if (/import \w* from 'prop-types'/.test(code)) {
      throw new Error(
        "TypeScript demo contains prop-types, please remove them",
      );
    }
    ...
    const codeWithPropTypes = injectPropTypesInFile({
      components: propTypesAST,
      target: code,
    });
    ...
    const codeWithoutTsIgnoreComments = codeWithPropTypes.replace(
      /^\s*\/\/ @ts-ignore.*$/gm,
      "",
    );
    ...
    await fs.promises.writeFile(
      jsPath,
      await prettierFormat(correctedLineEndings),
    );
