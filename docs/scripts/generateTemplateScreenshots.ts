...
const projects = {
  "material-ui": {
    input: path.join(
      process.cwd(),
      "docs/pages/material-ui/getting-started/templates",
    ),
  ...
  "joy-ui": {
    input: path.join(
      process.cwd(),
      "docs/pages/joy-ui/getting-started/templates",
    ),
    output: "docs/public/static/screenshots",
        ...
        async function captureDarkMode(outputPath: string) {
          ...
          if (btn) {
            if ((await btn.getAttribute("aria-haspopup")) === "true") {
              ...
              (await btn.getAttribute("aria-haspopup")) === "listbox"
            ) {
              await toggleMode();
              await page
                .getByRole("option")
        ...
        try {
          await Promise.resolve().then(() =>
            urls.reduce(async (sequence, aUrl) => {
              await sequence;
              await page.goto(`${host}${aUrl}?hideFrame=true`, {
              ...
              const toggleTheme = await page.$(
                '[data-screenshot="toggle-default-theme"]',
              );
              if (toggleTheme) {
                ...
                await captureDarkMode(
                  filePath.replace(".jpg", "-default-dark.jpg"),
                );
              }
