...
async function run() {
  try {
    ...
    iconList.forEach((icon) => {
      const synonymsIconStrings = synonyms[icon]
        ? synonyms[icon].split(" ")
        : [];
      ...
      const materialIconStrings = materialIcons[icon]
        ? materialIcons[icon].reduce(
            (tags, tag) => tags.concat(tag.split(" ")),
            [],
          )
        : [];
      ...
      mergedStrings = mergedStrings
        ...
        .filter(
          (tag) =>
            !mergedStrings.some((one) => one.includes(tag) && one !== tag),
        )
        .sort()
        .join(" ");
    ...
    console.log(
      `${Object.keys(npmPackageIcons).length} icons in @mui/material`,
    );
    console.log(
      `${Object.keys(materialIcons).length} icons in Material Design`,
    );
  } catch (err) {
    console.log("err", err);
    throw err;
