...
export default function TemplateCarousel() {
  ...
  const shadow = Object.keys(defaultTheme.vars.shadows).map(
    (key) => `--mui-shadows-${key}`,
  );
  const overlay = Object.keys(defaultTheme.vars.overlays).map(
    (key) => `--mui-overlays-${key}`,
  );
  const spacing = ["--mui-spacing"];
  const shape = ["--mui-shape-borderRadius"];
  const zIndex = Object.keys(defaultTheme.vars.zIndex).map(
    (key) => `--mui-zIndex-${key}`,
  );
  return (
    <ThemeProvider
