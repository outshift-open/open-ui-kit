...
function Heading(
  props: Pick<SectionTitleProps<ApiHeaderKeys>, "hash" | "level">,
) {
  const { hash, level = "h2" } = props;
  const t = useTranslate();
  return (
    <SectionTitle
      title={getTranslatedHeader(t, hash)}
      hash={hash}
      level={level}
    />
  );
...
export function ApiPage(props: ApiPageProps) {
  ...
  const description = t("api-docs.pageDescription").replace(
    /{{name}}/,
    pageContent.name,
  );
            ...
            >
              <div
                dangerouslySetInnerHTML={{
                  __html:
                    deprecationInfo || t("api-docs.defaultDeprecationMessage"),
                }}
              />
            </Alert>
          ) : null}
