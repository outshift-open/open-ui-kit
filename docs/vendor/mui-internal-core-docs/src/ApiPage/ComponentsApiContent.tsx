...
function getTranslatedHeader(t: Translate, header: string, title?: string) {
  ...
  return (
    translations[header] || (title && translations[title]) || title || header
  );
}
...
function Heading(props: SectionTitleProps) {
  ...
  return (
    <SectionTitle
      title={getTranslatedHeader(t, hash, title)}
      hash={hash}
      level={level}
    />
  );
...
export function ComponentsApiContent(props: ComponentsApiContentProps) {
  ...
  React.useEffect(() => {
    const anchor = router.asPath.includes("#")
    ...
    return (
      <React.Fragment key={`component-api-${key}`}>
        <MarkdownElement>
          ...
          <Heading
            title="import"
            hash={`${componentNameKebabCase}-import`}
            level="h3"
          />
          ...
          {imports.length > 1 && (
            <p
              dangerouslySetInnerHTML={{
                __html: t("api-docs.importDifference"),
              }}
            />
          )}
          <PropertiesSection
          ...
          <SlotsSection
            ...
            spreadHint={
              slotGuideLink &&
              t("api-docs.slotDescription").replace(
                /{{slotGuideLink}}/,
                slotGuideLink,
              )
            }
