    ...
    : [
        {
          ...
          children: [
            ...classes.map(({ key, hash: classHash }) => ({
              text: key,
              hash: componentName
                ? `${kebabCase(componentName)}-classes-${key}`
                : classHash,
              children: [],
            })),
          ],
...
export function getClassApiDefinitions(
  params: GetClassApiDefinitionsParams,
): ClassDefinition[] {
  ...
  return componentClasses.map((classDefinition) => {
    ...
    if (description.includes("{{conditions}}")) {
      if (!conditions) {
        throw new Error(
          errorMessage(componentName, classDefinition.className, "conditions"),
        );
      }
    ...
    if (description.includes("{{nodeName}}")) {
      if (!nodeName) {
        throw new Error(
          errorMessage(componentName, classDefinition.className, "nodeName"),
        );
      }
