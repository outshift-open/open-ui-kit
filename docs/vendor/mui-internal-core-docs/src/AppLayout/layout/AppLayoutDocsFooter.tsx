...
async function submitFeedback(
  ...
  try {
    const res = await fetch(
      `${window.location.origin}/.netlify/functions/feedback-management/`,
      {
        method: "POST",
...
export function AppLayoutDocsFooter(props: AppLayoutDocsFooterProps) {
  ...
  const [storedRating, setRating] = useLocalStorageState(
    `feedback-${activePage?.pathname}`,
  );
  const [comment, setComment] = React.useState("");
  const [loading, setLoading] = React.useState(false);
  const [snackbarOpen, setSnackbarOpen] = React.useState(false);
  const [snackbarMessage, setSnackbarMessage] = React.useState<string | false>(
    false,
  );
  ...
  React.useEffect(() => {
    const eventListener = (event: MouseEvent) => {
      const feedbackHash = (event.target as HTMLElement).getAttribute(
        "data-feedback-hash",
      );
      if (feedbackHash) {
        const section =
  ...
  const hidePagePagination =
    activePage === null || !(activePage as OrderedMuiPage).ordered;
          ...
          >
            ...
            <Tooltip title={t("feedbackYes")}>
              <IconButton
                ...
                sx={{
                  fontSize: 15,
                  color: rating === 1 ? "primary.main" : "text.secondary",
                }}
              >
                {rating === 1 && loading ? (
            ...
            <Tooltip title={t("feedbackNo")}>
              <IconButton
                ...
                sx={{
                  fontSize: 15,
                  color: rating === 0 ? "error.main" : "text.secondary",
                }}
              >
                {rating === 0 && loading ? (
                  <CircularProgress size={15} />
        ...
        >
          ...
          >
            <Link href="/" aria-label="Go to the homepage">
              <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
                <OpenUiKitLogomarkIcon height={24} width={24} />
                <Typography
                  sx={{ fontWeight: 800, color: "#00142b", fontSize: 14 }}
                >
                  Open UI Kit
                </Typography>
              </Stack>
            ...
            <Typography
              sx={{ color: "grey.500", fontSize: 13, opacity: "70%" }}
            >
              &bull;
            </Typography>
            <FooterLink
              ...
              rel="noopener"
            >
              GitHub <ArrowOutwardRoundedIcon />
            </FooterLink>
            <Typography
              sx={{ color: "grey.500", fontSize: 13, opacity: "70%" }}
            >
          ...
          <Stack spacing={1} direction="row" useFlexGap>
            <IconButton
              ...
              size="small"
            >
              <GitHubIcon fontSize="small" sx={{ color: iconColor }} />
            </IconButton>
            <IconButton
              target="_blank"
              rel="noopener"
