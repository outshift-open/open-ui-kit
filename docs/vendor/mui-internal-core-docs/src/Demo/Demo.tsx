...
})<{ hideToolbar?: boolean; bg?: string | boolean; hasAiSuggestion?: boolean }>(
  ({ theme }) => ({
    position: "relative",
    margin: "auto",
    display: "flex",
...
export function Demo(props: DemoProps) {
  ...
  if (process.env.NEXT_RUNTIME) {
    ...
    if (
      ...
      demoOptions.defaultCodeOpen === true
    ) {
      throw new Error(
        [
  ...
  React.useEffect(() => {
    ...
    if (
      navigatedDemoName &&
      (demoName === navigatedDemoName ||
        (anchorName !== null && anchorName === navigatedDemoName))
    ) {
      setCodeOpen(true);
    }
  }, [demoName, anchorName]);
  ...
  const [debouncedError, setDebouncedError] = React.useState<string | null>(
    null,
  );
  ...
  const handleTabChange: NonNullable<Tabs.Root.Props["onValueChange"]> = (
    value,
  ) => {
  ...
  const tabs = React.useMemo(() => {
    ...
    if (codeVariant === CODE_VARIANTS.TS && demo.moduleTS) {
      demoModule =
        demo.moduleTS === demo.module
          ? demoData.module!.replace(/\.js$/, ".tsx")
          : demo.moduleTS;
    }

    return [
      { module: demoModule, raw: demoData.raw },
                ...
                >
                  {demoOptions.disableLiveEdit || index > 0 ? (
                    <DemoCodeViewer
                      ...
                      copyButtonProps={
                        {
                          "data-ga-event-category": codeOpen
                            ? "demo-expand"
                            : "demo",
                      ...
                      sx={{
                        ...
                        "& pre": {
                          borderBottomLeftRadius: demoOptions.aiSuggestion
                            ? 0
                            : 12,
                          borderBottomRightRadius: demoOptions.aiSuggestion
                            ? 0
                            : 12,
                        },
                      }}
