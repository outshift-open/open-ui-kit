...
type Demo = Pick<
  DemoData,
  "productId" | "raw" | "codeVariant" | "relativeModules"
>;
...
export function SandboxDependencies(
  ...
  function extractDependencies() {
    ...
    demo.relativeModules?.forEach(({ raw }) =>
      extractImportedDependencies(raw),
    );
  ...
  if (
    csbConfig?.fallbackDependency &&
    !dependencies[csbConfig.fallbackDependency.name]
  ) {
    dependencies[csbConfig.fallbackDependency.name] =
      csbConfig.fallbackDependency.version;
  }
