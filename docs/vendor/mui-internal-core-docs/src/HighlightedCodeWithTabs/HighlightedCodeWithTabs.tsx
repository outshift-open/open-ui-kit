...
export const CodeTabList = styled(Tabs.List)<{
...
export const CodeTab = styled(Tabs.Tab)<{
  ownerState: { mounted: boolean; contained?: boolean };
}>(({ theme }) => ({
  variants: [
    {
...
export function HighlightedCodeWithTabs(
  ...
  const availableTabs = React.useMemo(() => {
    ...
    if (storageKey === "package-manager") {
      result.sort(
        (a, b) =>
          (PACKAGE_MANAGER_ORDER.get(a) ?? Infinity) -
          (PACKAGE_MANAGER_ORDER.get(b) ?? Infinity),
      );
    }
    return result;
  }, [storageKey, tabs]);
  ...
  const handleChange: NonNullable<Tabs.Root.Props["onValueChange"]> = (
    value,
  ) => {
