...
describe("docs getDependencies helpers", () => {
  ...
  it("should return correct title", () => {
    expect(
      pageToTitle({ pathname: "/docs/src/pages/components/button/button.md" }),
    ).to.equal("Button");
    expect(pageToTitle({ pathname: "/components" })).to.equal("Components");
    expect(
      pageToTitle({ pathname: "/customization/how-to-customize" }),
    ).to.equal("How to customize");
  });

  it("should remove `react-` prefix", () => {
    expect(
      pageToTitle({ pathname: "/docs/pages/material/react-buttons.js" }),
    ).to.equal("Buttons");
  });
});
