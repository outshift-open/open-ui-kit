 ...
 */
export default function useLazyCSS(
  href: string,
  before: string,
    ...
    fetch(href, { signal: abortController.signal })
      ...
      .catch((error) => {
        ...
        if (error !== "useEffect" && error.name !== "AbortError") {
          const isOptionalRemoteCssFetch =
            error instanceof TypeError && href.startsWith("https://");

          if (
            process.env.NODE_ENV !== "production" &&
            !isOptionalRemoteCssFetch
          ) {
          ...
          styleElement = loadCSS(
            href,
            document.querySelector(before) as HTMLElement,
          );
        }
      });
