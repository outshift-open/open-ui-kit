...
describe("getProductInfoFromUrl", () => {
  it("should handle Material UI", () => {
    ...
    expect(
      getProductInfoFromUrl("/zh/material-ui/react-button/"),
    ).to.deep.equal({
      productCategoryId: "core",
      productId: "material-ui",
    });
  ...
  it("should ignore anchor", () => {
    expect(
      getProductInfoFromUrl(
        "/material-ui/react-app-bar/#app-bar-with-responsive-menu",
      ),
    ).to.deep.equal({
      productCategoryId: "core",
      productId: "material-ui",
  ...
  it("should handle MUI X Data Drid", () => {
    expect(
      getProductInfoFromUrl("/x/react-data-grid/components"),
    ).to.deep.equal({
      productCategoryId: "x",
  ...
  it("should handle MUI X Date Pickers", () => {
    expect(
      getProductInfoFromUrl("/x/react-date-pickers/components"),
    ).to.deep.equal({
      productCategoryId: "x",
  ...
  it("should handle MUI X Scheduler", () => {
    expect(
      getProductInfoFromUrl("/x/react-scheduler/components"),
    ).to.deep.equal({
      productCategoryId: "x",
  ...
  it("should handle MUI X", () => {
    expect(
      getProductInfoFromUrl("/x/migration/migration-data-grid-v5/"),
    ).to.deep.equal({
      productCategoryId: "x",
  ...
  it("should handle Toolpad Studio", () => {
    expect(
      getProductInfoFromUrl("/toolpad/studio/getting-started/first-app/"),
    ).to.deep.equal({
      productCategoryId: "toolpad",
