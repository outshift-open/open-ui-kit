...
function isTypeJSXElementLike(
  ...
  if (symbol) {
    ...
    return (
      ...
      name.endsWith(".JSX.Element") ||
      name.endsWith(".ReactElement") ||
      name.endsWith(".ReactNode")
    );
  }
  ...
  if (type.isUnion()) {
    return type.types.every(
      ...
      (subType) =>
        subType.flags & ts.TypeFlags.Null ||
        isTypeJSXElementLike(subType, project),
    );
  }
...
function getJSXLikeReturnValueFromFunction(
  type: ts.Type,
  project: TypeScriptProject,
) {
  return type
    .getCallSignatures()
    .filter((signature) =>
      isTypeJSXElementLike(signature.getReturnType(), project),
    );
}
...
function parseFunctionComponent({
  ...
  Object.keys(squashedParsedComponent.props).forEach((propName) => {
    squashedParsedComponent.props[propName].onlyUsedInSomeSignatures =
      squashedParsedComponent.props[propName].signatures.length <
      signatures.length;
  });
...
>) {
  ...
  if (
    ...
    if (
      (ts.isArrowFunction(potentialComponent) ||
        ts.isFunctionExpression(potentialComponent)) &&
...
export function getPropsFromComponentNode({
  ...
  if (
    ...
    getJSXLikeReturnValueFromFunction(
      project.checker.getTypeAtLocation(node.name),
      project,
    ).length > 0
  ) {
