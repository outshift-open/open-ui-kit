/*
 * Copyright 2025 Cisco Systems, Inc. and its affiliates
 *
 * SPDX-License-Identifier: Apache-2.0
 */

import { SvgIcon, SvgIconProps } from "@mui/material";
import {
  grey0,
  illustrationInfoGradientEnd,
  illustrationInfoGradientMiddle,
  illustrationInfoGradientStart,
  purple600,
} from "@/theme/style/color-palette";

export const Info = (props: SvgIconProps) => {
  return (
    <SvgIcon {...props}>
      <svg
        width="224"
        height="224"
        viewBox="0 0 224 224"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <rect
          opacity="0.3"
          x="52.7144"
          y="80.75"
          width="27"
          height="120"
          rx="13.5"
          transform="rotate(25 52.7144 80.75)"
          fill={purple600}
        />
        <rect
          opacity="0.3"
          x="46.0356"
          y="2"
          width="27"
          height="90"
          rx="13.5"
          transform="rotate(25 46.0356 2)"
          fill={purple600}
        />
        <rect
          opacity="0.3"
          x="188.116"
          y="119"
          width="27"
          height="48.1906"
          rx="13.5"
          transform="rotate(25 188.116 119)"
          fill={purple600}
        />
        <path
          d="M151 53.5H73C67.8283 53.5 62.8684 55.5545 59.2114 59.2114C55.5545 62.8684 53.5 67.8283 53.5 73V151C53.5 156.172 55.5545 161.132 59.2114 164.789C62.8684 168.446 67.8283 170.5 73 170.5H151C156.172 170.5 161.132 168.446 164.789 164.789C168.446 161.132 170.5 156.172 170.5 151V73C170.5 67.8283 168.446 62.8684 164.789 59.2114C161.132 55.5545 156.172 53.5 151 53.5Z"
          fill="url(#paint0_linear_23095_15669)"
        />
        <path
          d="M99.8139 84.5375C99.8139 82.7377 100.386 80.9783 101.457 79.4818C102.528 77.9853 104.051 76.819 105.833 76.1302C107.614 75.4414 109.575 75.2612 111.466 75.6124C113.357 75.9635 115.095 76.8302 116.458 78.1028C117.822 79.3755 118.75 80.997 119.127 82.7622C119.503 84.5274 119.31 86.3571 118.572 88.0199C117.834 89.6827 116.584 91.104 114.981 92.1039C113.377 93.1038 111.492 93.6375 109.564 93.6375C106.978 93.6375 104.498 92.6788 102.67 90.9722C100.841 89.2656 99.8139 86.951 99.8139 84.5375ZM119.314 130.424V111.837C119.314 108.821 118.03 105.927 115.744 103.794C113.459 101.661 110.359 100.462 107.126 100.462C105.399 100.46 103.727 101.028 102.406 102.066C101.085 103.104 100.2 104.545 99.9082 106.134C99.6163 107.722 99.9362 109.356 100.811 110.746C101.686 112.135 103.06 113.191 104.689 113.726V132.312C104.689 135.329 105.973 138.223 108.259 140.356C110.544 142.489 113.644 143.687 116.876 143.687C118.603 143.69 120.275 143.122 121.597 142.084C122.918 141.046 123.803 139.605 124.095 138.016C124.386 136.428 124.067 134.794 123.192 133.404C122.317 132.015 120.943 130.959 119.314 130.424Z"
          fill={grey0}
        />
        <defs>
          <linearGradient
            id="paint0_linear_23095_15669"
            x1="53.5"
            y1="112"
            x2="166.469"
            y2="90.6615"
            gradientUnits="userSpaceOnUse"
          >
            <stop stopColor={illustrationInfoGradientStart} />
            <stop
              offset="0.498521"
              stopColor={illustrationInfoGradientMiddle}
            />
            <stop offset="1" stopColor={illustrationInfoGradientEnd} />
          </linearGradient>
        </defs>
      </svg>
    </SvgIcon>
  );
};
